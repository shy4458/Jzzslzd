package com.bksx.jzzslzd.bo;

/**
 * 房屋信息
 */
public class Fwxx {
    //房屋查询
    private String fzxm;//房主姓名
    private String ssjb;//单位级别
    private String dwid;//登录单位id
    private String fzsfzhm;//房主身份证号码
    private String fwlb;//房屋类别(02:出租房屋 其他编码：非出租房屋)
    private String pcsdm;//派出所编码
    private String fwbh;//房屋编号
    private String fzxzdxxdz;//房屋现住地详细地址
    private String fzlxdh;//房主联系电话
    private String fwdz;//房屋地址
    private String szdxzqh;//房屋所在地行政区划
    private String fwzbh;//服务站编号
    private String ssxqbh;//所属辖区编号
    private String tbr;//填表人
    private String glybm;//管理员编码
    private String djrq;//登记日期
    private String djr;//登记人
    private String[][] fwxx;//房屋查询结果
    private String fwsl;//房屋数量


    public String getGlybm() {
        return glybm;
    }

    public void setGlybm(String glybm) {
        this.glybm = glybm;
    }

    public String getDjrq() {
        return djrq;
    }

    public void setDjrq(String djrq) {
        this.djrq = djrq;
    }

    public String getDjr() {
        return djr;
    }

    public void setDjr(String djr) {
        this.djr = djr;
    }



    public String getFwsl() {
        return fwsl;
    }

    public void setFwsl(String fwsl) {
        this.fwsl = fwsl;
    }

    public String[][] getFwxx() {
        return fwxx;
    }

    public void setFwxx(String[][] fwxx) {
        this.fwxx = fwxx;
    }

    public String getFwbh() {
        return fwbh;
    }

    public void setFwbh(String fwbh) {
        this.fwbh = fwbh;
    }

    public String getFzxzdxxdz() {
        return fzxzdxxdz;
    }

    public void setFzxzdxxdz(String fzxzdxxdz) {
        this.fzxzdxxdz = fzxzdxxdz;
    }

    public String getFzlxdh() {
        return fzlxdh;
    }

    public void setFzlxdh(String fzlxdh) {
        this.fzlxdh = fzlxdh;
    }

    public String getFwdz() {
        return fwdz;
    }

    public void setFwdz(String fwdz) {
        this.fwdz = fwdz;
    }

    public String getSzdxzqh() {
        return szdxzqh;
    }

    public void setSzdxzqh(String szdxzqh) {
        this.szdxzqh = szdxzqh;
    }

    public String getFwzbh() {
        return fwzbh;
    }

    public void setFwzbh(String fwzbh) {
        this.fwzbh = fwzbh;
    }

    public String getSsxqbh() {
        return ssxqbh;
    }

    public void setSsxqbh(String ssxqbh) {
        this.ssxqbh = ssxqbh;
    }

    public String getTbr() {
        return tbr;
    }

    public void setTbr(String tbr) {
        this.tbr = tbr;
    }

    public String getFzxm() {
        return fzxm;
    }

    public void setFzxm(String fzxm) {
        this.fzxm = fzxm;
    }

    public String getSsjb() {
        return ssjb;
    }

    public void setSsjb(String ssjb) {
        this.ssjb = ssjb;
    }

    public String getDwid() {
        return dwid;
    }

    public void setDwid(String dwid) {
        this.dwid = dwid;
    }

    public String getFzsfzhm() {
        return fzsfzhm;
    }

    public void setFzsfzhm(String fzsfzhm) {
        this.fzsfzhm = fzsfzhm;
    }

    public String getFwlb() {
        return fwlb;
    }

    public void setFwlb(String fwlb) {
        this.fwlb = fwlb;
    }



    public String getPcsdm() {
        return pcsdm;
    }

    public void setPcsdm(String pcsdm) {
        this.pcsdm = pcsdm;
    }
}

